...
export function DeliveryProductPicker({ products, onSelect }: DeliveryProductPickerProps) {
  ...
  if (groups.length === 0) {
    return <p className="text-center text-sm text-gray-400 dark:text-gray-500">No products yet — add one first.</p>
  }

  return (
    <div className="flex flex-col gap-6">
      <p className="text-sm text-gray-500 dark:text-gray-400">Which product arrived?</p>
      {groups.map((group) => (
